			...
			<div className="allSectionTasks">
				{sectionsFiltered.map((item: ICompany, index: number) => {
					if (index < _MAX_LENGTH) {
						let cheked: boolean = this.state.campaign ? this.state.campaign.id === item.id : false;
						return (
							<div>
								<div
									key={item.id}
									className={cheked ? "sectionTaskItem pressed" : "sectionTaskItem"}
				...
				<div className={sectionsFiltered.length > _MAX_LENGTH ? "hoveredTask" : "hoveredTask hide"}>
	...
	getFilteredSection(): ICompany[] {
		...
		return filter(
			...
			({ dateStart, dateFinish }: ICompany) =>
				current.diff(moment.unix(dateStart), "day") >= 0 && current.diff(moment.unix(dateFinish), "hour") <= 0
		);
	}
}
