...
function getMonth(monthNumber) {
    var month = [];
    var week = [];
    var day = moment_1["default"]("2019-" + monthNumber + "-01");
    var weekDay = day.day();
    week[weekDay] = day;
    if (weekDay > 0) {
        var first = moment_1["default"](day);
        do {
            first = moment_1["default"](first).subtract(1, "days");
            week[first.day()] = first;
        } while (first.day() > 0);
    }
    do {
        day = moment_1["default"](day).add(1, "days");
        weekDay = day.day();
        week[weekDay] = day;
        if (weekDay >= 6) {
            month.push(week);
    ...
    if (weekDay < 6) {
        var last = moment_1["default"](day);
        do {
            last = moment_1["default"](last).add(1, "days");
            week[last.day()] = last;
        } while (last.day() < 6);
    }
    month.push(week);
